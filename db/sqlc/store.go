...
// Store provides all functions to execute db transactions
type Store struct {
	*Queries
	db *sql.DB
}
...
// execTransaction execute a function within a database transaction
...
// get transactionKey as an empty struct
...
func (store *Store) TransferTransaction(ctx context.Context, arg TransferTxParams) (TransferTxResult, error) {
	...
	err := store.execTransaction(ctx, func(q *Queries) error {
		...
		result.Transfer, err = q.CreateTransfer(
			ctx,
			//lint:ignore S1016 we want to have this type explicitly
